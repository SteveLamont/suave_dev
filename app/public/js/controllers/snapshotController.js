function SnapshotController()
{
  // bind event listeners to button clicks //
	...
	//spl:070318 Added to support date/time filter restore.
      if(snapshotPara.datetime_filters != "None" && !filter){
          $("#panel-para").append("<tr>");
          $("#panel-para").append("<h4>Date/Time filters:</h4>");
	  for(var i = 0; i < snapshotPara.datetime_filters.length; i++){
              var filter = snapshotPara.datetime_filters[i];
              if(i>0){
		  $("#panel-para").append("<br>"+filter.facet+":");
              }else{
		  $("#panel-para").append(filter.facet+"(");
		  ...
		  if ( j > 0 )
		      $("#panel-para").append( ","  );
		  if ( filter.value[j].value != undefined )
		      $("#panel-para").append( filter.value[j].value  );
		  else {

		      var begin =
			  new Date( filter.value[j].startDate ).toLocaleDateString()
		      var end =
			  new Date( filter.value[j].endDate ).toLocaleDateString()
		      $("#panel-para").append( begin + "-" + end );
      ...
      $.ajax({
        ...
        success: function(data){
          ...
          $(".panel-title").append("File: "+data.file+" User: "+data.user);
            if( data.num_filters != "None" ||
		data.string_filters != "None" ||
		data.datetime_filters != "None" ){
