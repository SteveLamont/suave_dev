...
function HomeController()
{
	...
	$.ajax({
		...
		$('.modal-select-collection .modal-dialog .modal-body').prepend(
			...
			'Tags: number, date, long, link, ordinal, multi, textlocation, info, hiddenmore, hidden, sortquan'+
			'</div>'+
			'</div>'+
			'<div class="dropdown">'+
	...
	if(!survey.dzc){
		...
		success: function(data){
			...
			//spl:060318: Skip the column if it's "#img"
			if ( column[i] != "#img" ) {
	...
	//spl:082218 -- id.slice(-1) was returning only the last digit
	//of the identifier.
	var s = id.split( "-" ).slice(-1)
	surveys[s].view = view;
//	surveys[id.slice(-1)].view = view;
	...
	//of the identifier.
	var s = id.split( "-" );
	var file = surveys[s.slice(-1)].name;
//	var file = surveys[id.slice(-1)].name;
	
	var that = this;
	$('.modal-confirm').modal('toggle');
	$('.modal-confirm .modal-header h3').text('Warning!');
	//spl:082218 -- Tell the user more specifically what we're
	//about to delete.
	$('.modal-confirm p').append("Are you sure to delete survey '" +
				     file +
				     "' along with all its annotations?");
