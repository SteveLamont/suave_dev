	...
	this.bucketized = false;
        ...
        $.subscribe("/PivotViewer/Views/Canvas/Zoom", function(evt) {
            ...
            if (newWidth < that.width || that.scale <= 1) {
                ...
                that.currentOffsetX =
		    evt.x - ( ( ( evt.x - that.currentOffsetX ) /
				oldScale ) * that.scale );
                that.currentOffsetY =
		    evt.y - ( ( ( evt.y - that.currentOffsetY ) /
				oldScale ) * that.scale );
		
		//spl:080118 -- Removed titleSpace from calculation to
		//fix "drift up" issue when selected image is zoomed out.
                that.canvasHeightUIAdjusted =
		    newHeight - ( ( that.offsetY /
//		    newHeight - ( ( ( that.offsetY + that.titleSpace ) /
				    oldScale ) * that.scale );
		...
		//spl:080118 -- Changed to prevent bogus recalculation
		//of image positions, especially on smaller screens.
//                that.resetUISettings();
    ...
    recalibrateUISettings: function() {
        this.rowscols =
	    this.getTileDimensions( this.columnWidth - 2,
				    this.canvasHeightUIAdjusted - this.offsetY,
				    this.maxRatio,
				    this.bigCount,
				    this.rowscols);
    },
    ...
    createUI: function() {
        ...
        for (var i = 0; i < this.tiles.length; i++) {
            ...
            // if ( tile.filtered &&
	    // 	 (Settings.showMissing || !tile.missing))
	    // 	continue;
            ...
            for (var l = 0; l < tloc.length; l++) {
                ...
                // var theta =
		//     Math.atan2(tloc[l].y - (this.currentHeight / 2),
		// 	       tloc[l].x - (this.currentWidth / 2))
                // tloc[l].destinationx =
		//     Math.abs(this.currentWidth * Math.cos(theta) +
		// 	     (this.currentWidth / 2));
                // tloc[l].destinationy =
		//     this.currentHeight * Math.sin(theta) +
		//     (this.currentHeight / 2);
		tloc[l].destinationx = tloc[l].destinationy = Number.MAX_SAFE_INTEGER;
    ...
    setTilePositions: function( rowscols,
				offsetX, offsetY,
				initTiles, keepColsRows ) {

        var columns =
	    (keepColsRows && this.rowscols) ?
	    this.rowscols.Columns : rowscols.Columns;
        if (!keepColsRows)
	...
	if ( !this.bucketized ) {
		...
		for (var i = 0; i < bucket.tiles.length; i++) {
		    ...
		    var tloc = tile._locations[tile.locationIndex++];
		    ...
		    tloc.starty = tloc.y;
                    ...
                    tloc.destinationx =
			(b * this.columnWidth) +
			(currentColumn * rowscols.TileMaxWidth) +
			offsetX;
                    tloc.destinationy =
			this.canvasHeightUIAdjusted -
			rowscols.TileHeight -
			(currentRow * rowscols.TileHeight) + offsetY;
		    
		    tloc.bucket = b;
		    tloc.bucket_row = currentRow;
		    tloc.bucket_column = currentColumn;
		...
		for (var i = 0; i < bucket.tiles.length; i++) {
			...
			tileLocation.destinationx =
			    (b * this.columnWidth) +
			    (currentColumn * rowscols.TileMaxWidth) + offsetX;
	...
	//spl:072318 -- Added test for filterList length.
        if ( ( filterList.length == 0 ) ||
	     ( filterList[0].item.getFacetByName(sortCategory) == undefined) ) {

            var bkts = [];
            bktNo = new PivotViewer.Models.Bucket( "(no info)", "(no info)" );

            bkts.push(bktNo);
            bkts.ids = [];

	    //spl:072318 -- Test for "showMissing" and return values
	    //if it's set.
		...
		for ( var i = 0; i < filterList.length; i++) {
		    ...
		    bkts.ids[filterList[i].item.id] = 0;
	    ...
	    //spl:070118 Format the string to a fixed precision.
	    var places =
		Math.floor( Math.log10( Math.abs( bucketSize ) ) );
	    var fmt =
		"%" + ( ( places < 0 ) ? ( "." + ( -places ) ) : "" ) + "f";
            ...
            for (var i = 0; i < numBuckets; i++) {
                ...
                bkts[i] =
		    new PivotViewer.Models.Bucket(start, sprintf( fmt, start ),
						  end, sprintf( fmt, end ) );
                // bkts[i] =
		//     new PivotViewer.Models.Bucket(start, start.toString(),
		// 				  end, end.toString());
            }
            var i = 0;
...
//                bkts.ids = [];
    ...
    centerOnTile: function(tile,x,y) {

        var item = tile.item;
        var location = tile._locations[tile.selectedLoc];
        ...
        var origProportion;
	var img_ratio = TileController._imageController.getRatio(item.img);
        if (tile.height / canvasHeight >
	    (tile.height / img_ratio) / canvasWidth)
            origProportion = tile.origheight / canvasHeight;
        else
	    origProportion = tile.origwidth / canvasWidth;

        if (this.selected == null)
	    PV.zoom(Math.round((0.75 / origProportion) * 2));

	//spl:072818 -- added bucket_row and bucket_column fields to
	//the tile object.  Calculation moved from above since after
	//PV.zoom() the objects may have been reordered.

	var tlocs = tile._locations;
	var tw = tile.origwidth;
	...
	for ( var i = 0; i < tlocs.length; i++ ) {
	    ...
	    if ( ( tloc.x <= x ) && ( x < ( tloc.x + tw ) ) &&
		 ( tloc.y <= y ) && ( y < ( tloc.y + th ) ) ) {
	...
	//spl: end fix.
        ...
        this.currentOffsetX =
	    (this.width / 2) -
	    (this.rowscols.TileMaxWidth * col) -
	    (this.rowscols.TileMaxWidth / 2) - padding;
        this.currentOffsetY =
	    this.height / 2 -
	    this.canvasHeightUIAdjusted +
	    this.rowscols.TileHeight / 2 +
	    row * this.rowscols.TileHeight;

        this.setTilePositions( this.rowscols,
			       this.currentOffsetX,
			       this.currentOffsetY,
			       true, true );
    ...
    handleClick: function(evt) {
        ...
        if (tile != null) {
            if (this.selected != tile) {
                ...
                this.centerOnTile(tile,evt.x,evt.y);
