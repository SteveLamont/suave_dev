...
window.onerror = function (message, url, line) {
    ...
    console.debug(message, url, line);
...
(function ($) {
    ...
    PV.filterCollection = function (filterChange) {
        ...
        if (filterChange == undefined) {
            ...
            for (var i = 0; i < checked.length; i++) {
                ...
                if (category.isString() || category.isLocation()) {
                    ...
                    else if (_stringFilters[name] != undefined) {
                        stringFilter = {
                            facet: name,
                            value: [],
                            index: i
        ...
        if (_longstringFilters != null ||
            _globalStringFilter != null ||
            _numericFilters.length != 0 ||
            _stringFilters.length != 0 ||
            _datetimeFilters.length != 0)
        ...
        if (_options.authoring == true) {
            ...
            //spl:070318 Missing filter added.
            PARA.datetime_filters = _datetimeFilters;
        }
        ...
        LoadSem.acquire(function (release) {
            ...
            if (category.isString() || category.isLocation()) {
                ...
                if (!_listObj[PV.cleanName(category.name)])
                    _listObj[PV.cleanName(category.name)] =
                        new List("pv-cat-" +
    ...
    $.subscribe("/PivotViewer/ImageController/Collection/Loaded", function (event) {
        ...
        if (_jupyterEnable === true) {
            ...
            //user does not own survey so ask them to clone first
            $('.pv-toolbarpanel-viewcontrols').append("<div id='pv-ownership' class='pv-modal-dialog modal-xl'><div><h2>You Don't Own This Survey!</h2><div id='pv-ownership-text'>&nbsp;</div></div></div>");


            //modal to login
            $('.pv-toolbarpanel-viewcontrols').append("<div id='pv-login' class='pv-modal-dialog modal-xl'><div><h2>Login</h2><div id='pv-login-text'>&nbsp;</div></div></div>");

            //modal for gmail input
            $('.pv-toolbarpanel-viewcontrols').append("<div id='pv-gmail-input' class='pv-modal-dialog modal-xl'><div><h2>Input Gmail Address</h2><div id='pv-gmail-input-text'>&nbsp;</div></div></div>");


            //modal for clone survey and jupyterhub options
        ...
        var params = "none";
        var dzc_file;
        if (options.dzc == "undefined") { //when no dzc is given (ie using default)
            dzc_file = "";
        }
        else {
        ...
        uri_params = uri_params + "&" + "csv=" + file;
        uri_params = uri_params + "&" + "params=" + params;
        uri_params = uri_params + "&" + "dzc=" + dzc_file;
        uri_params = uri_params + "&" + "activeobject=" + active_object;


        var suave_local_jupyter_url = "http://suave-jupyterhub.com/user/zeppelin-v/notebooks/SuaveDispatch.ipynb" + uri_params;
        var optiputer_url_base = "https://suave-jupyter.nautilus.optiputer.net/user/";
        var optiputer_url_notebook = "/notebooks/suave/SuaveDispatch.ipynb";
        var mybinder_url = "https://mybinder.org/v2/gh/suave-ucsd/jupyter-suave/master?urlpath=/notebooks/SuaveDispatch.ipynb" + encodeURIComponent(uri_params);


        //html code for jupyter modal
        var j_modelHTML = "<p><button id='pv-j-suave-local-open'>Run Jupyter on SuAVE Data Server</button></p>";
        j_modelHTML += "<p><button id='pv-j-optiputer-open'>Run Jupyter on CHASE_CI</button></p>";
        //j_modelHTML += "<p><button id = 'pv-j-static-mybinder-open'>Run Jupyter on MyBinder(Static)</button></p>";
        //j_modelHTML += "<p><button id ='pv-j-dynamic-mybinder-open'>Run Jupyter on MyBinder(Dynamic)</button></p>";

        //j_modelHTML = "<div class='dropdown'><button class='btn btn-secondary dropdown-toggle' type='button' id='dropdownMenuButton' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'>Run Jupyter with...</button><span class='caret'></span><div class='dropdown-menu' aria-labelledby='dropdownMenuButton'><a class='dropdown-item' href=''>SuAVE-Jupyterhub</a><a class='dropdown-item' href=''>Optiputer(Nautilus)</a><a class='dropdown-item' href=''>MyBinder</a></div></div>";
        ...
        //build login form
        var loginHTML = "<hr><p><h3>If this is your first time logging into SuAVE on this device, please turn your pop-up blocker off.</h3><form id='pv_login_form' action='/' method='POST'>";
        loginHTML += "<fieldset><p class='subheading'>Username:</p><input type='text' name='user' id='user' required='required'>";
        loginHTML += "<p class='subheading'>Password:</p><input type='password' name='pass' id='pass' required='required'>";
        loginHTML += "<input type='hidden' name='remember-me' id='remember-me' value='true'>"
        loginHTML += "<div class='form-buttons'><button id='login-submit' type='submit' class='btn btn-primary'>Login</button></div></fieldset></form></p>";
        ...
        ownershipHTML += "<p><button id='pv-j-cancel-owernship'>Cancel</button></p>";
        ...
        var gmailinputHTML = "<hr><p><h3>Enter your gmail address for use with Optiputer and proceed to Jupyter.</h3></p>";
        ...
        gmailinputHTML += "<div class='form-buttons'><button id='gmail-submit' type='submit' class='btn btn-primary'>Launch Jupyter</button></div></fieldset></form></p>";
        ...
        clone_form.submit(function (e) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    var clone_url = base_url + "=" + username + "_" + new_survey_name + ".csv&views=1110001&view=grid";
        ...
        $('.pv-infopanel-details').on("click", '.detail-item-value-filter', function (e) {
            ...
            if (!isDoubleClicked($('.pv-infopanel-details'))) {
                ...
                $.publish("/PivotViewer/Views/Item/Filtered", [{
                    ...
                    min: _filtered_detail[$(this).attr('pv-detail-item-value')],
                    values: null
                }]);
    ...
    PV._setCustomDateRange = function (facet, start, end) {
        ...
        PV._setCustomDate(facet, "Start", start);
        PV._setCustomDate(facet, "Finish", end);
